...
public static class ImageHeader
{
    ...
    public static Size GetPixelSize(byte[] buffer, string mime = null)
    {
        ...
        return GetPixelSizeWithFormat(new MemoryStream(buffer), mime, false);
    }
    ...
    /// <param name="leaveOpen">If false, the passed stream will get disposed</param>
    /// <returns>The dimensions of the specified image.</returns>    
    public static Size GetPixelSizeWithFormat(Stream input, bool leaveOpen = true)
    {
        return GetPixelSizeWithFormat(input, null, leaveOpen);
    }
    ...
    {
        return GetPixelSizeWithFormat(input, mime, leaveOpen);
    }
    ...
    /// <param name="leaveOpen">If false, the passed stream will get disposed</param>
    /// <returns>The dimensions of the specified image.</returns>    
    public static Size GetPixelSizeWithFormat(Stream input,string mime, bool leaveOpen = true)
    {
    ...
    private static Size DecodeJpeg(BinaryReader reader)
    {
        ...
        while (true)
        {
            ...
            else if (state == "sof")
            {
                ...
                {
                    state = c[0] >= 0xC0 && c[0] <= 0xC3 || c[0] >= 0xC5 && c[0] <= 0xC7 || c[0] >= 0xC9 && c[0] <= 0xCB || c[0] >= 0xCD && c[0] <= 0xCF
                        ? "readsize"
                        : c[0] == 0xFF ? "sof" : "skipframe";
                }
            }
