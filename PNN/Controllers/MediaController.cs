...
public class MediaController : ControllerBase
{
    ...
    public MediaController(ILogger<MediaController> logger, IMediaService mediaService,
        IMediaTypeResolver mediaTypeResolver, MediaFileDbContext mediaFileDbContext, IIdentityService identityService)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> CopyDirectories()
    {
        ...
        {
            string targetFilePath = Path.Combine("D:\\leader\\LearnToLeader\\TestMediaFile\\App_data\\Images_copy",
                file.Name);
        ...
        foreach (string filename in Directory.EnumerateFiles(startDirectory))
        {
            using (FileStream sourceStream = System.IO.File.Open(filename, FileMode.Open))
            {
                using (FileStream destinationStream =
                       System.IO.File.Create(Path.Combine(endDirectory, Path.GetFileName(filename))))
                {
    ...
    )
    {
        ...
        for (int i = 0; i < numFiles; i++)
        {
            ...
            try
            {
                ...
                var mediaFile = await _mediaService.SaveFileAsync(filePath, uploadFile.OpenReadStream(), isTransient,
